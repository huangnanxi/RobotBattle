...
public class RobotFightBattle extends AbstractRobotBattle {
    ...
    protected void doBeforeRound(Integer currentRound) {
        ...
        int noNodeNum = 0;
        for (int x = 0; x < mapLatticeHasElement.length; x++) {
            for (int y = 0; y < mapLatticeHasElement.length; y++) {
                if (!mapLatticeHasElement[x][y]) {
                    noNodeNum++;
                }
            }
        }
        int survivalNum = 0;
        for (AbstractRobot robot : fightElementsData.robots) {
        ...
        int turnRoundNum = noNodeNum <= survivalNum ? 10 : ((FightGameControlConfig) gameControlConfig).turnRoundNum;
        if (currentRound % turnRoundNum == 0) {
